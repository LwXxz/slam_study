...
// img path
string imgPath1 = "../data/1.png";
string imgPath2 = "../data/2.png";
...
{
    // load image
    cv::Mat first_image = cv::imread(imgPath1, 0);
    cv::Mat second_image = cv::imread(imgPath2, 0);
    cout << first_image.rows << " " << first_image.cols << endl;
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    ...
    for (auto &kp : keypoints)
    {
        ...
        for (int dx = -half_patch_size; dx < half_patch_size; ++dx)
        {
            ...
            {
                uchar pixel = img.at<uchar>(kp.pt.y + dy, kp.pt.x + dx); // 取像素值
                m10 += dx * pixel;
                m01 += dy * pixel;
            }
        }
        ...
        for (int i = 0; i < 8; i++)        // 8*32=256位的描述子
        {
            ...
            {
                int idx_pq = i * 32 + k;  // 每次循环用掉4个值，一共256行数据，一次循环32行
                ...
                // 旋转角度为特征点到质心构成的向量的角度
                // p.x=x1-x2、p.y=y1-y2
                // p -> pp, q -> qq 转化为图片中的位置, pp图片1 qq图片2
